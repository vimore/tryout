...
public class ArcSightListner extends SyslogAlertListener  {

    private final Logger LOGGER = LoggerFactory.getLogger(ArcSightListner.class);
    ...
    public ArcSightListner(int listenerId, String hostName, int port, String transportProtocol, AlertAuditLogger alertLogger) {
        super("arcsight", transportProtocol, hostName, port);
    ...
    public void sendAlert(Object obj) throws Exception {
        LOGGER.info("Sending to ArcSight...");
            ...
            try {
                for(CEF cef : cefs) {
                    ...
                    if(message != null) {
                        ...
                        LOGGER.debug("CEF logged to ArcSight("+this.transportProtocol+" "+this.hostName+":"+this.port+") => " + message);
                        LOGGER.debug("CEF: "+message);
                    }
                }

            } catch (Exception e) {
                LOGGER.error("Error occurred while logging to ArchSight => ", e);
                throw new RuntimeException("Error occurred while logging to ArchSight => " + e);
            }
            syslogSender.flush();
        }
    }
