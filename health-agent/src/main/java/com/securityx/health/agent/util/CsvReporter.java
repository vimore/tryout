...
/**
 * RAM: a copy of com.codahale.metrics.CsvReporter for adding only one method reportEvent().
 * I wish CsvReporter was inheritable
 */
...
public class CsvReporter extends ScheduledReporter {
    ...
    public void reportEvent(long timestamp, String name, Event event){
        try {
            ...
            if (fileAlreadyExists || file.createNewFile()) {
                try (PrintWriter out = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file, true), UTF_8))) {
                    if (!fileAlreadyExists) {
                        out.println("timestamp," + String.join(",", event.getHeaderLabels()));
                    }
                    out.printf(locale, String.format(locale, "%d,%s%n", timestamp, event.getLineFormat()), event.getValues().toArray());
                }
            }
        } catch (IOException e) {
    ...
    private void report(long timestamp, String name, String header, String line, Object... values) {
        try {
            ...
            final boolean fileAlreadyExists = file.exists();
            if (fileAlreadyExists || file.createNewFile()) {
                // no need a finally block and manage the resource
                try (PrintWriter out = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file, true), UTF_8))) {
                    if (!fileAlreadyExists) {
