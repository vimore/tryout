...
import com.securityx.mef.log.mapreduce.LogParsingReducer;
 ...
 */
public class LogFileParsingJob extends Configured implements Tool {

    //    public static final String MORPHLINE_OUTPUT = "file_morphline_output";
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LogFileParsingJob.class);
    private static final String HDFS = "hdfs://";
    private static final String HDFSRAWLOGBASEDIR = "/securityx/flume/livestream";
    private static final String DEFAULTPROPERTIESFILE = "logfileparsingjob.properties";
    private static Options options = new Options();
    private Class inputFormatClass = NonSplittableTextInputFormat.class;
    //private Configuration conf;
    private Properties props;
    private String optLogSource = null;
    private boolean purgeDestDir = false;
    private String morphlineFile = "iamdbmef-ms-ad.conf";
    private String morphlineId = "msad-csvde_parser";
    ...
    public LogFileParsingJob() {
        super();
    }

    public LogFileParsingJob(String id, Properties props) {
        super();
        this.props = props;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        int res = ToolRunner.run(conf, new LogFileParsingJob(), args);
        System.exit(res);
    }
    ...
    private int doJob() throws Exception {
        ...
        System.out.println("cluster : " + fs.getCanonicalServiceName());

        String tmpOutputDir = HdfsFileUtils.genJobTmpDir(fs);
        System.out.println("tmpOutputDir: " + tmpOutputDir);
        ...
        Path inputPath = new Path(jobConfig.getInputPaths());
        FileInputFormat.addInputPaths(job, jobConfig.getInputPaths());
        FileInputFormat.setInputPathFilter(job, FileInputPathFilter.class);
        ...
        if (r == 0) {
            if (MefParser.moveToFinalDest(fs, new Path(tmpOutputDir), this.jobConfig,
                    job.getJobID().toString(), this.purgeDestDir) != 0)
    ...
    public int run(String[] args) throws Exception {
        ...
        if (LogParsingJobProperties.validateLogFileParsingJob(conf)) {
            ...
            String split = LogParsingJobProperties.get(conf, LogParsingJobProperties.SPLITABLE);
            if (null != split && "true".equals(split.toLowerCase())) {
                inputFormatClass = TextInputFormat.class;
            }

            String logSource = LogParsingJobProperties.get(conf, LogParsingJobProperties.LOGSOURCE);
            if (null != logSource) {
                optLogSource = logSource;
            }

            this.purgeDestDir = LogParsingJobProperties.getBoolean(conf, LogParsingJobProperties.DOPURGEDST, false);


            processingName = this.getClass().getName()+"-"+jobConfig.getInputPaths();

            return doJob();
        } else {
            System.err.println(LogParsingJobProperties.usageLogFileParsingJob());
