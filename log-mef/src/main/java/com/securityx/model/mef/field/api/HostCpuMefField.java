...
public enum HostCpuMefField implements SupportedFormat {
...
//Arrays.<MefFieldConstrait>asList(new MefMilliSecTimeStampConstraint()), LongMarshall.class,"setEndTime", Long.class, true),
    ...
    memConsumption(1, MefFieldType.data, "device", "device host mem consumption in %",
            Arrays.<MefFieldConstrait>asList(new MefFloatConstraint()), FloatMarshall.class, "setMemConsumption", CharSequence.class, true),
    ...
    private HostCpuMefField(int numberOfValues,
    ...
    private void initSupportedFormatFields() {
        for (HostCpuMefField field : values()) {
    ...
    public SupportedFormat getByPrettyName(String prettyName) {
        ...
        try {
            format = HostCpuMefField.valueOf(prettyName);
