...
import com.securityx.model.external.ExternalFormatToFlowMefFormat;
 ...
 */
public class ExternalFormatToFlowMefFormatCommandBuilder implements CommandBuilder {

  private static final String commandName = "flowMefFormat";
  ...
  static class ExternalFormatToMefFormatCommand implements Command {
    ...
    public boolean process(Record record) {
      ...
      ExternalFormatToFlowMefFormat externalFormatToMefFormat = getFormat(record);
      if (externalFormatToMefFormat == null) {
        ...
        for (ExternalFieldsToFlowMefFields externalFieldsToMefFields : externalFormatToMefFormat.getExternalFieldsToMefFields()) {
          for (String externalFieldName : externalFieldsToMefFields.getExternalFieldNames()) {
            ...
            if (got != null && ! got.isEmpty()) {
              ...
              FlowMefField mefField = externalFieldsToMefFields.getMefField(externalFieldName);
              if (mefField != null) {
                if (logger.isDebugEnabled()) {
        ...
        record.put(formatIdFromFieldName, "FlowMef");
    ...
    private ExternalFormatToFlowMefFormat getFormat(Record record) {
      if (record == null) {
        return null;
      }
      ...
      if (got.size() == 1) {
        ...
        try {
          return ExternalFormatToFlowMefFormat.valueOf(externalFormatId);
