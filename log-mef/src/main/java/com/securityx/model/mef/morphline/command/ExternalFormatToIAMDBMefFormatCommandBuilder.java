...
import com.securityx.model.external.ExternalFormatToIAMDBMefFormat;
 ...
 */
public class ExternalFormatToIAMDBMefFormatCommandBuilder implements CommandBuilder {

  private static final String commandName = "iamDbMefFormat";
  ...
  public Command build(Config config, Command parent, Command child, MorphlineContext context) {
    ...
    return new ExternalFormatToIAMDBMefFormatCommand(config, context, parent, child,
            formatIdFromFieldName, discardUnmappableFields, discardOriginalMappedFields);
  }

  static class ExternalFormatToIAMDBMefFormatCommand implements Command {

    private final Logger logger = LoggerFactory.getLogger(ExternalFormatToIAMDBMefFormatCommand.class);
    ...
    public ExternalFormatToIAMDBMefFormatCommand(Config config, MorphlineContext context,
    ...
    public boolean process(Record record) {
      ...
      ExternalFormatToIAMDBMefFormat externalFormatToMefFormat = getFormat(record);
      if (externalFormatToMefFormat == null) {
        ...
        for (ExternalFieldsToIAMDBMefFields externalFieldsToMefFields : externalFormatToMefFormat.getExternalFieldsToMefFields()) {
          for (String externalFieldName : externalFieldsToMefFields.getExternalFieldNames()) {
            ...
            if (got != null && ! got.isEmpty()) {
              ...
              IAMDBMefField mefField = externalFieldsToMefFields.getMefField(externalFieldName);
              if (mefField != null && ! mefField.name().equals(externalFieldName)){
                if (logger.isDebugEnabled()) {
                  logger.debug("converting " + externalFormatToMefFormat.name() + "." + externalFieldName+ " to "+mefField.name());
                }
                ...
                for (Object o : got) {
                  ...
                  if(logger.isDebugEnabled())
                     logger.debug("Meffield is null for "+externalFieldName);
              }
            }
          }
        }
        ...
        record.put(formatIdFromFieldName, "IAMDBMef");
    ...
    private ExternalFormatToIAMDBMefFormat getFormat(Record record) {
      if (record == null) {
        return null;
      }
      ...
      if (got.size() == 1) {
        ...
        try {
          return ExternalFormatToIAMDBMefFormat.valueOf(externalFormatId);
