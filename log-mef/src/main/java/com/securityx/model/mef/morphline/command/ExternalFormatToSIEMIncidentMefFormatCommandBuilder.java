...
import com.securityx.model.external.ExternalFormatToSIEMIncidentMefFormat;
 ...
 */
public class ExternalFormatToSIEMIncidentMefFormatCommandBuilder implements CommandBuilder {

  private static final String commandName = "sIEMIncidentMefFormat";
  ...
  static class ExternalFormatToMefFormatCommand implements Command {
    ...
    public boolean process(Record record) {
      ...
      ExternalFormatToSIEMIncidentMefFormat externalFormatToMefFormat = getFormat(record);
      if (externalFormatToMefFormat == null) {
        ...
        for (ExternalFieldsToSIEMIncidentMefFields externalFieldsToMefFields : externalFormatToMefFormat.getExternalFieldsToMefFields()) {
          for (String externalFieldName : externalFieldsToMefFields.getExternalFieldNames()) {
            ...
            if (got != null && ! got.isEmpty()) {
              ...
              SIEMIncidentMefField mefField = externalFieldsToMefFields.getMefField(externalFieldName);
              if (mefField != null) {
                if (logger.isDebugEnabled()) {
        ...
        record.put(formatIdFromFieldName, "SIEMIncidentMef");
    ...
    private ExternalFormatToSIEMIncidentMefFormat getFormat(Record record) {
      if (record == null) {
        return null;
      }
      ...
      if (got.size() == 1) {
        ...
        try {
          return ExternalFormatToSIEMIncidentMefFormat.valueOf(externalFormatId);
