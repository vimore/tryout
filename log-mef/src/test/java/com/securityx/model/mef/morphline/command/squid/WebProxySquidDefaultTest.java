...
public class WebProxySquidDefaultTest extends LogCollectionAbstractTest {

  private Logger logger = LoggerFactory.getLogger(MefBluecoatScriptTest.class);

  public WebProxySquidDefaultTest(String name) {
    super(name);
    this.morphlineId = "squidfromaccesslog";
    this.confFile = "webproxy-squid.conf";
  }

  @Test
  public void test_http200() throws FileNotFoundException {
    String line = "1392121179.563    515 81.56.112.95 TCP_MISS/200 74364 GET http://www.cloudera.com/content/support/en/downloads/download-components/download-products.html? - DIRECT/65.50.196.141 text/html";
  ...
  public void test_http404() throws FileNotFoundException {
    String line = "1392052987.632     27 81.56.112.95 TCP_MISS/404 1015 GET http://nine-eyes.herokuapp.com/? - DIRECT/54.243.157.122 text/html";
    boolean result = doTest(line);
    assertEquals(true, result);
    Record res = this.outCommand.getRecord(0);
  ...
  public void test_ssl() throws FileNotFoundException {
    String line = "1392068647.385 117047 81.56.112.95 TCP_MISS/200 4814 CONNECT hivedata.jiveon.com:443 - DIRECT/23.218.66.131 -";
    boolean result = doTest(line);
    assertEquals(true, result);
    Record res = this.outCommand.getRecord(0);
    Assert.assertEquals("cefSignatureId", "200", res.get("cefSignatureId").get(0));
  ...
  public void test_httpcustomport() throws FileNotFoundException {
    String line = "1392123313.883    511 81.56.112.95 TCP_MISS/200 1257 GET http://portquiz.net:8080/ - DIRECT/178.33.250.62 text/html";
    boolean result = doTest(line);
    assertEquals(true, result);
    Record res = this.outCommand.getRecord(0);
    Assert.assertEquals("cefSignatureId", "200", res.get("cefSignatureId").get(0));
  ...
  public void test_denied() throws FileNotFoundException {
    String line = "1392123510.839      0 81.56.112.95 TCP_DENIED/403 3109 GET http://portquiz.net:666/ - NONE/- text/html";
    boolean result = doTest(line);
    assertEquals(true, result);
    Record res = this.outCommand.getRecord(0);
  ...
  public void test_badhost() throws FileNotFoundException {
    String line = "1392051431.025      6 81.56.112.95 TCP_MISS/503 3480 GET http://www.notrefamille.comhttp//actualite.benchmark.fr/depeche/une/312c4e2df631c40dcf9845720ff1b9ea789460bcdiaporama480x320.jpg - DIRECT/www.notrefamille.comhttp text/html";
    boolean result = doTest(line);
    assertEquals(true, result);
    Record res = this.outCommand.getRecord(0);
