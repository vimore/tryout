package com.securityx.model.mef.morphline.command.wsa;
...
public class WebProxyWSASyslogTest extends LogCollectionAbstractTest {

  private Logger logger = LoggerFactory.getLogger(WebProxyWSASyslogTest.class);

  public WebProxyWSASyslogTest(String name) {
    super(name);
    this.morphlineId = "wsafromrsyslog";
    this.confFile = "webproxy-wsa.conf";
    OutUtils.printOut("constructor called");
    try {
    	setUp();
    } catch(Exception e) {
  ...
  public void test_http200() throws FileNotFoundException {
	...
	String line = "<38>Sep  8 13:23:46 10.44.90.25 <14>Sep 08 13:23:38 ACCESS_LOGS: Info: 1473366217.541 1300 10.40.5.210 TCP_CLIENT_REFRESH_MISS/200 1040 POST http://intelliconnect.cch.com/scion/secure/ctx_4260093/treeModelService.rpc - DIRECT/intelliconnect.cch.com text/plain DEFAULT_CASE_12-DefaultGroup-DefaultGroup-DefaultGroup-NONE-NONE-DefaultGroup <IW_busi,3.9,0,\"-\",0,0,0,1,\"-\",-,-,-,\"-\",1,-,\"-\",\"-\",-,-,IW_busi,-,\"Unknown\",\"-\",\"Unknown\",\"Unknown\",\"-\",\"-\",6.40,0,Local,\"Unknown\",\"-\",-,\"-\",-,-,\"-\",\"-\"> -";	
    boolean result = doTest(line);
    assertEquals(true, result);
    Record res = this.outCommand.getRecord(0);
    OutUtils.printOut(res.toString());
    Assert.assertEquals("cefSignatureId", "200", res.get("cefSignatureId").get(0));
    //Assert.assertEquals("destinationAddress", "www.msftncsi.com", res.get("destinationAddress").get(0));
    Assert.assertEquals("destinationNameOrIp", "intelliconnect.cch.com", res.get("destinationNameOrIp").get(0));
    Assert.assertEquals("deviceAction", "DIRECT", res.get("deviceAction").get(0));
    Assert.assertEquals("devicePolicyAction", "TCP_CLIENT_REFRESH_MISS", res.get("devicePolicyAction").get(0));
    
    Assert.assertEquals("bytesIn", 1040, res.get("bytesIn").get(0));
    Assert.assertEquals("requestMethod", "POST", res.get("requestMethod").get(0));
    Assert.assertEquals("requestPath", "/scion/secure/ctx_4260093/treeModelService.rpc", res.get("requestPath").get(0));
    Assert.assertEquals("requestScheme", "http", res.get("requestScheme").get(0));
    Assert.assertEquals("responseContentType", "text/plain", res.get("responseContentType").get(0));
    Assert.assertEquals("sourceNameOrIp", "10.40.5.210", res.get("sourceNameOrIp").get(0));
    Assert.assertEquals("sourceUserName", "-", res.get("sourceUserName").get(0));
    Assert.assertEquals("startTime", 1473366217541L, res.get("startTime").get(0));
    Assert.assertEquals("logSourceType", "WebProxyMef", res.get("logSourceType").get(0));
    Assert.assertEquals("externalLogSourceType", "Cisco_WSA", res.get("externalLogSourceType").get(0));        
  }
/*
  @Test
...
//  }
  ...
  /*
  	public static void main(String[] args) {
  		WebProxyWSASyslogTest wt = new WebProxyWSASyslogTest("WSATest");
