...
/* Compute (1 - (1 - SLD_anomaly)(1 - UA_anomaly)(1 - method_anomaly))(Alexa_anomaly) */
/* One entry per hour, c_ip cs_host. Pick least common request_method and user_agent. group/sort/limit */
/*request_method_totals = FOREACH (GROUP hosts_sld_request_method_and_user_agent BY (c_ip, cs_host, hour, request_method)) GENERATE 
    FLATTEN(group) AS (c_ip, cs_host, hour, request_method),
    COUNT_STAR(hosts_sld_request_method_and_user_agent) AS total;   
rarest_request_method = FOREACH (GROUP request_method_totals BY (c_ip, cs_host, hour)) {
    sorted = ORDER request_method_totals BY total;
    top_1 = LIMIT sorted 1;
    GENERATE FLATTEN(group) AS (c_ip, cs_host, hour), FLATTEN(top_1.request_method) AS request_method;
}    
user_agent_totals = FOREACH (GROUP hosts_sld_request_method_and_user_agent BY (c_ip, cs_host, hour, user_agent)) GENERATE
    FLATTEN(group) AS (c_ip, cs_host, hour, user_agent),
    COUNT_STAR(hosts_sld_request_method_and_user_agent) AS total;
rarest_user_agent = FOREACH (GROUP user_agent_totals BY (c_ip, cs_host, hour)) {
    sorted = ORDER user_agent_totals BY total;
    top_1 = LIMIT sorted 1;
    GENERATE FLATTEN(group) AS (c_ip, cs_host, hour), FLATTEN(top_1.user_agent) AS user_agent;
}*/
