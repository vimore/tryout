...
rmf /tmp/beacons_weekly.txt
rmf /tmp/beacon_samples_weekly.avro

bluecoat = LOAD '/securityx/web_proxy_mef/$year/$month/$day/$hour/*' USING AvroStorage();
...
bluecoat = FOREACH bluecoat GENERATE values#'sourceNameOrIp' AS c_ip:chararray, 
                                     ...
                                     ISOToWeek(UnixToISO((long)values#'startTime')) AS week,
                                     rawLog AS raw_log:chararray,
                                     values#'requestClientApplication' AS user_agent,
                                     values#'requestMethod' AS request_method;
bluecoat = FILTER bluecoat BY cs_host IS NOT NULL AND c_ip IS NOT NULL AND week IS NOT NULL;
...
beacon_samples = FOREACH (GROUP bluecoat BY (c_ip, cs_host, week)) {
    sorted = ORDER bluecoat BY date_time;
    first_3 = LIMIT sorted 3;
    GENERATE FLATTEN(group) AS (c_ip, cs_host, week), FLATTEN(first_3.(date_time, raw_log)) AS (date_time, raw_log);
};
beacon_samples = FOREACH beacon_samples GENERATE date_time, week, c_ip, cs_host, raw_log;
STORE beacon_samples INTO '/tmp/beacon_samples_weekly.avro' USING AvroStorage();
...
trimmed_bluecoat = FOREACH bluecoat GENERATE c_ip, cs_host, week, request_method, user_agent;
sld_cip = STREAM trimmed_bluecoat THROUGH second_level_command AS (source:chararray, sld:chararray, week:chararray, request_method:chararray, user_agent:chararray);
/* Ugly fix for null tuples coming out of last line */
sld_cip = FILTER sld_cip BY (source IS NOT NULL) AND (sld IS NOT NULL) AND (week IS NOT NULL);

/* Now take the unique sources per 2nd level domain, and count them - giving you the degree of the 2nd level domain */
sld_degree = FOREACH (GROUP sld_cip BY (sld, week)) {
	unique_cips = DISTINCT sld_cip.source;
	GENERATE FLATTEN(group) AS (sld, week), 
...
sld_degree_max_counts = FOREACH (GROUP sld_degree BY week) GENERATE group AS week, 
                                                                    FLATTEN(sld_degree.(sld, counts)) AS (sld, counts), 
                                                                    MAX(sld_degree.counts) as max_val;

/* Now compute the SLD anomaly score */                                                                    
sld2anomaly = FOREACH sld_degree_max_counts GENERATE week,
                                                     sld, 
...
alexa_score = FOREACH with_alexa GENERATE sld, week, (ranking IS NOT NULL ? (.1 * LOG10(ranking) + 0.3) : 1.0) AS score;
alexa_score = DISTINCT alexa_score;

/* Now compute the user-agent anomaly score */
/* Get counts of user agents per edge */
user_agent_per_edge_week = FOREACH (GROUP sld_cip BY (source, sld, week, user_agent)) GENERATE 
    FLATTEN(group) AS (source, sld, week, user_agent),
    COUNT_STAR(sld_cip) AS total;
user_agent_totals = FOREACH (GROUP user_agent_per_edge_week BY (week, user_agent)) GENERATE 
    FLATTEN(group) AS (week, user_agent),
    COUNT_STAR(user_agent_per_edge_week) AS total;
user_agent_max = FOREACH (GROUP user_agent_totals BY week) GENERATE group AS week, 
                                                                    FLATTEN(user_agent_totals.user_agent) AS user_agent, 
                                                                    (double)MAX(user_agent_totals.total) AS max_val:double;
user_agent_totals_with_max = JOIN user_agent_totals BY (week, user_agent), user_agent_max BY (week, user_agent);  
user_agent_anomalies = FOREACH user_agent_totals_with_max GENERATE 
    user_agent_totals::week AS week,
...
method_per_edge_week = FOREACH (GROUP sld_cip BY (source, sld, week, request_method)) GENERATE
    FLATTEN(group) AS (source, sld, week, request_method),
    COUNT_STAR(sld_cip) AS total;
request_method_totals = FOREACH (GROUP method_per_edge_week BY (week, request_method)) GENERATE 
    FLATTEN(group) AS (week, request_method),
    COUNT_STAR(method_per_edge_week) AS total;
request_method_max = FOREACH (GROUP request_method_totals BY week) GENERATE group AS week,
                                                                            FLATTEN(request_method_totals.request_method) AS request_method,
                                                                            MAX(request_method_totals.total) AS max_val;
request_method_totals_with_max = JOIN request_method_totals BY (week, request_method), request_method_max BY (week, request_method);
request_method_anomalies = FOREACH request_method_totals_with_max GENERATE 
    request_method_totals::week AS week,
...
links = FOREACH bluecoat GENERATE date_time, week, c_ip, cs_host, request_method, user_agent;
grouped_by_ip2hostpair = GROUP links BY (c_ip, cs_host, week, request_method, user_agent);
sorted_grouped_by_ip2hostpair = FOREACH grouped_by_ip2hostpair {
    sorted_datetimes = ORDER links BY date_time;
    GENERATE FLATTEN(group) AS (c_ip, cs_host, week, request_method, user_agent), sorted_datetimes.(date_time) AS sorted_datetimes;
};
sorted_grouped_by_ip2hostpair = FOREACH sorted_grouped_by_ip2hostpair GENERATE c_ip, cs_host, week, request_method, sorted_datetimes, user_agent;
...
hosts_and_sld_anomaly = JOIN ip2host_riqr BY (sld, week), sld2anomaly BY (sld, week), alexa_score BY (sld, week);
hosts_and_sld_anomaly = FOREACH hosts_and_sld_anomaly GENERATE ip2host_riqr::c_ip AS c_ip,
                                                               ip2host_riqr::cs_host AS cs_host,
                                                               ip2host_riqr::week AS week,
...
hosts_and_sld_with_request_method = JOIN hosts_and_sld_anomaly BY (week, request_method), request_method_anomalies BY (week, request_method); 
hosts_and_sld_with_request_method = FOREACH hosts_and_sld_with_request_method GENERATE hosts_and_sld_anomaly::c_ip AS c_ip,
                                                                                     ...
                                                                                     hosts_and_sld_anomaly::week AS week,
...
hosts_sld_request_method_and_user_agent = JOIN hosts_and_sld_with_request_method BY (week, user_agent), user_agent_anomalies BY (week, user_agent);
hosts_sld_request_method_and_user_agent = FOREACH hosts_sld_request_method_and_user_agent GENERATE hosts_and_sld_with_request_method::c_ip AS c_ip,
                                                                                                   ...
                                                                                                   hosts_and_sld_with_request_method::week AS week,
...
STORE hosts_and_risks INTO '/tmp/beacons_weekly.txt';
