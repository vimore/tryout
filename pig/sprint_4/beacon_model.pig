...
bluecoat = FOREACH bluecoat GENERATE sourceNameOrIp AS c_ip:chararray, 
                                     destinationNameOrIp AS cs_host:chararray,
                                     startTimeISO AS date_time,
                                     ISOToHour(startTimeISO) AS hour,
                                     rawLog AS raw_log:chararray,
                                     requestClientApplication AS user_agent,
                                     requestMethod AS request_method;
bluecoat = FILTER bluecoat BY cs_host IS NOT NULL AND c_ip IS NOT NULL AND hour IS NOT NULL;


beacons = LOAD 'beacons_hourly.txt' USING AvroStorage();
